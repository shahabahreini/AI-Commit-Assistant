    ...
    public async startSubscription(): Promise<void> {
        try {
            ...
            const action = await vscode.window.showInformationMessage(
                `🚀 Checkout opened for ${selectedProduct.name}!\n\nComplete your purchase in the browser window. Your Pro features will be activated automatically once payment is confirmed.`,
                { modal: false },
            ...
            } else if (action === 'Open Checkout Again') {
                await vscode.env.openExternal(vscode.Uri.parse(checkoutUrl));
            }
