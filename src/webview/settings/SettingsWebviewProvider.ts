                ...
                <script>
                    ...
                    document.getElementById('commitVerbose').addEventListener('change', (event) => {
                        ...
                        if (window.currentSettings) {
                            window.currentSettings.commit = window.currentSettings.commit || {};
                            window.currentSettings.commit.verbose = event.target.checked;
                        }
                        console.log('Commit verbose setting changed (not saved until Save button is clicked)');
                    });

                    document.getElementById('promptCustomizationEnabled').addEventListener('change', (event) => {
                        // Update local state only, don't auto-save
                        if (window.currentSettings) {
                            window.currentSettings.promptCustomization = window.currentSettings.promptCustomization || {};
                            window.currentSettings.promptCustomization.enabled = event.target.checked;
                        }
                        console.log('Prompt customization setting changed (not saved until Save button is clicked)');
                    });

                    document.getElementById('showDiagnostics').addEventListener('change', (event) => {
                        // Update local state only, don't auto-save
                        if (window.currentSettings) {
                            window.currentSettings.showDiagnostics = event.target.checked;
                        }
                        console.log('Show diagnostics setting changed (not saved until Save button is clicked)');
                    });

                    document.getElementById('apiProvider').addEventListener('change', (event) => {
                        const selectedProvider = event.target.value;
                        // Update UI only, don't auto-save
                        // Changes will only be saved when Save button is clicked
                        if (window.currentSettings) {
                            window.currentSettings.apiProvider = selectedProvider;
                            // Update UI to reflect provider change
                            updateVisibleSettings();
                        }
                        
                        // Log the change but don't save
                        console.log('Provider changed to ' + selectedProvider + ' (changes not saved until Save button is clicked)');
                    });
                </script>
            </body>
            </html>
